...
test(
  'it cannot run a grouping map over the items with the returns of the callback is not an array with 2 elements',
  () => {
    const users = [{}];
    expect(() => {
...
describe('it can filter out duplicate elements to ensure that array elements are unique', () => {
  ...
  test('with the item is an object', () => {
    expect(_arr([{ id: 1 }, { id: 2 }, { id: 1 }]).unique('id').get()).toEqual([{ id: 1 }, { id: 2 }]);
  });
});

describe('it can convert the array to options of a selection', () => {
  test('with the item is primitive type', () => {
    expect(_arr(['Nam', 'John Doe']).toSelectOptions())
      .toEqual([{ value: 0, label: 'Nam' }, { value: 1, label: 'John Doe' }]);
  });

  test('with the item is an object', () => {
    const users = [{ id: 1, name: 'Nam' }, { id: 2, name: 'John Doe' }];
    expect(_arr(users).toSelectOptions(['id', 'name']))
      .toEqual([{ value: 1, label: 'Nam' }, { value: 2, label: 'John Doe' }]);
  });
});
